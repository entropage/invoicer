...
def create_test_suite():
    ...
    if token:
        ...
        suite.addTest(unittest.FunctionTestCase(lambda: test_simple_command_injection(token)))
        suite.addTest(unittest.FunctionTestCase(lambda: test_pdf_command_injection(token)))
        suite.addTest(unittest.FunctionTestCase(lambda: test_ping_command_injection(token)))
...
def list_tests():
    ...
    jwt_tester = JWTVulnerabilityTest()
    for name in ['test_normal_flow', 'test_token_forgery', 'test_cross_environment', 'test_token_expiration_bypass']:
        ...
        if '.' in args.test:
            ...
            if args.test.startswith('test_jwt_'):
                ...
                if test_func.__module__ == 'test_command_injection':
                    token = login()
                    if token:
                        suite = unittest.TestSuite([unittest.FunctionTestCase(lambda: test_func(token))])
                    else:
                        print("Error: Could not run command injection test due to login failure")
                        return
                else:
                    suite = unittest.TestSuite([unittest.FunctionTestCase(test_func)])
            else:
