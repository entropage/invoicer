...
# Test authentication
test_endpoint "/api/auth/register" "POST" "{\"username\":\"${TEST_USERNAME:-test}\",\"password\":\"${TEST_PASSWORD:-test123}\"}"
token=$(curl -s -X POST "${BASE_URL}/api/auth/login" -H "Content-Type: application/json" -d "{\"username\":\"${TEST_USERNAME:-test}\",\"password\":\"${TEST_PASSWORD:-test123}\"}" | jq -r '.token')
