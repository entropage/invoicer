...
class TestServer:
    ...
    def start(self):
        try:
            ...
            try:
                test_conn = socket.create_connection(
                ...
                try:
                    test_response = requests.get(
                        f"http://localhost:{self.port}/test", timeout=1
                    )
                    print(
                        f"[TestServer] Self-test request successful: {test_response.status_code}"
                    )
                except Exception as e:
                    print(f"[TestServer] Warning: Self-test request failed: {e}")
            except Exception as e:
                print(
                    f"[TestServer] Warning: Could not verify server is listening: {e}"
                )
        except Exception as e:
...
class SSRFTest(unittest.TestCase):
    ...
    def test_01_local_logo(self):
        ...
        test_data["invoice"]["logo"] = (
            "data:image/png;base64," + b64encode(b"test").decode()
        )
        ...
        self.assertEqual(
            response.json()["invoiceId"], "SSRF-TEST-LOCAL", "Invoice ID mismatch"
        )
    ...
    def test_02_remote_logo(self):
        ...
        self.assertEqual(response.status_code, 200, "Invoice creation failed")
        self.assertEqual(
            response.json()["invoiceId"], "SSRF-TEST-REMOTE", "Invoice ID mismatch"
        )
    ...
    def test_03_internal_network(self):
        ...
        self.assertEqual(
            response.json()["invoiceId"], "SSRF-TEST-MONGO", "Invoice ID mismatch"
        )
        ...
        self.assertEqual(
            response.json()["invoiceId"], "SSRF-TEST-INTERNAL", "Invoice ID mismatch"
        )
    ...
    def test_04_cloud_metadata(self):
        ...
        self.assertEqual(
            response.json()["invoiceId"], "SSRF-TEST-AWS", "Invoice ID mismatch"
        )
    ...
    def test_05_protocols(self):
        ...
        for i, protocol in enumerate(protocols):
            ...
            self.assertIn(
                response.status_code, [200, 500], f"Protocol test failed for {protocol}"
            )
            if response.status_code == 200:
                self.assertEqual(
                    response.json()["invoiceId"],
                    f"SSRF-TEST-PROTO-{i+1}",
                    "Invoice ID mismatch",
                )
    ...
    def test_06_ip_ranges(self):
        ...
        for i, ip in enumerate(ip_ranges):
            ...
            self.assertIn(
                response.status_code, [200, 500], f"IP range test failed for {ip}"
            )
            if response.status_code == 200:
                self.assertEqual(
                    response.json()["invoiceId"],
    ...
    def test_07_ssrf_verification(self):
        ...
        print(
            f"[Test] Making SSRF test request to {test_data['invoice']['logoUrl']}..."
        )
        ...
        for i in range(10):  # Try for 10 seconds
            ...
            if i == 5:  # Check halfway through
                try:
                    test_response = requests.get(
                        f"http://localhost:{TEST_PORT}/test", timeout=1
                    )
                    print(
                        f"[Test] Server is still up (status: {test_response.status_code})"
                    )
                except Exception as e:
...
def main():
    ...
    parser.add_argument(
        "--test", "-t", help="Run a specific SSRF test (e.g. test_01_local_logo)"
    )
    parser.add_argument(
        "--list", "-l", action="store_true", help="List available SSRF tests"
    )
