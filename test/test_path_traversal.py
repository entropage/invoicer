    ...
    def test_01_simple_path_traversal(self):
        ...
        for target in self.LINUX_TARGETS:  # Container is Linux
            ...
            self.assertIn(
                ...
                [200, 403, 404, 500],
                f"Unexpected status code for {target}",
            )
    ...
    def test_03_template_traversal(self):
        ...
        for target in self.LINUX_TARGETS:
            traversal = "../" * 5 + target.lstrip(
                "/"
            )  # Fewer ../ needed as we start in templates/
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run path traversal vulnerability tests."
    )
    parser.add_argument(
        "--test", "-t", help="Run a specific test (e.g. test_01_simple_path_traversal)"
    )
    parser.add_argument(
